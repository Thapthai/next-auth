...
interface CreateItemCategoryFormProps {
    isVisible: boolean;
    saleOfficeData: any[];
    departmentData: any[];
    materialData: any[];
...
export default function CreateItemCategoryForm({
    isVisible,
    saleOfficeData,
    departmentData,
    materialData,
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/item-categories`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="mt-6 p-4 border rounded shadow bg-white space-y-3">
            <div className="flex justify-between items-center">
                <h2 className="text-lg font-bold text-gray-800">{t('createNewItemCategory')}</h2>
                <Button
                    variant="ghost"
                    size="sm"
                    onClick={handleClose}

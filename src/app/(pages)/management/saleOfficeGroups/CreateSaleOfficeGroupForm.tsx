...
interface CreateSaleOfficeGroupFormProps {
    isVisible: boolean;
    saleOfficeData: any[];
...
export default function CreateSaleOfficeGroupForm({
    isVisible,
    saleOfficeData,
    ...
    onStart,
    onError
}: CreateSaleOfficeGroupFormProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sale-office-groups`, {
                method: 'POST',
                headers: {
            ...
            if (!res.ok) {
                throw new Error('Failed to create sale office group');
            }
            ...
            console.error('Create sale office group error:', err);
            if (onError) onError();
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="mt-6 p-4 border rounded shadow bg-white space-y-3">
            <div className="flex justify-between items-center">
                <h2 className="text-lg font-bold text-gray-800">{t('createNewSaleOfficeGroup')}</h2>
                <Button
                    variant="ghost"
                    size="sm"
                    onClick={handleClose}

...
interface ItemCategoryPriceDetailProps {
    itemCategoryPrice: ItemCategoryPrice;
    isVisible: boolean;
...
export default function ItemCategoryPriceDetail({
    itemCategoryPrice,
    isVisible,
    itemCategoryData,
    onClose,
    ...
    useEffect(() => {
        if (itemCategoryPrice) {
            setForm({
                item_category_id: itemCategoryPrice.item_category_id || 0,
                price: itemCategoryPrice.price || 0,
                description: itemCategoryPrice.description || '',
                status: itemCategoryPrice.status ?? true
            });
        }
    }, [itemCategoryPrice]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/item-category-prices/${itemCategoryPrice.id}`, {
                method: 'PATCH',
                headers: {
            ...
            if (!res.ok) {
                throw new Error('Failed to update item category price');
            }

            onSuccess();
            onClose();
        } catch (err) {
            console.error('Update item category price error:', err);
            if (onError) onError();
        } finally {
            setLoading(false);
        }
